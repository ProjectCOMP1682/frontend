...
const HistoryTradePost = () => {
    ...
    let handleOnClickExport =async ()=>{
        ...
        if(res.errCode === 0){
            let formatData = res.data.map((item)=> {
                let obj = {
                    'Package name': item.packageOrderData.name,
                    'Transaction code': item.id,
                    'Package type': item.packageOrderData.isHot == 0 ? "Normal type" : "Featured type",
                    'Quantity purchased': item.amount,
                    'Total Price': item.packageOrderData.price + " USD",
                    'Purchaser': item.userOrderData.firstName + ' ' + item.userOrderData.lastName,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Giả định rằng getData là một hàm async
        ...
        <div className="col-12 grid-margin">
            <div className="card bg-white p-6 rounded-lg shadow-lg">
                <div className="card-body">
                    <div className="mb-4">
                    <h4 className="text-xl font-semibold">Post Package Payment History</h4>
                    <button
                                            ...
                                            data.map((item, index) => {
                                                return (
                                                    <tr key={index}  className="bg-white hover:bg-gray-50">
                                                        ...
                                                        <td className="border border-gray-300 px-4 py-2">{item.packageOrderData.name}</td>
                                                        <td className="border border-gray-300 px-4 py-2">{item.id}</td>
                                                        <td className="border border-gray-300 px-4 py-2">{item.packageOrderData.isHot == 0 ? "Normal type" : "Featured type"}</td>
                                                        <td className="border border-gray-300 px-4 py-2">{item.amount}</td>
                                                        <td className="border border-gray-300 px-4 py-2" style={{ textAlign: 'right' }}>{item.packageOrderData.price} USD</td>
                                                        <td className="border border-gray-300 px-4 py-2">{item.userOrderData.firstName + ' ' + item.userOrderData.lastName }</td>
